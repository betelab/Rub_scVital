...
def testClustAndStats(adata, umapKey, neighborsKey, pcaRep, 
	...
	if(not pd.isna(cellLabel)):
		adata = checkPairsAddSimple(adata, batchLabel, cellLabel)
...
def getAllStats(dirName,datasetName,paramName):
    ...
    allStats = pd.DataFrame(np.zeros((4,len(latents))),index=["Runtime\n(min)  ","ARI","FM","LSS"],columns=latents)
...
def vizAllStats(allStats, h=2, w=4, name="", outDir=False):
...
def makeLegend(ctVals, btVals, cellTypeColorDict, batchColorDict, name="", outDir=False):
    ...
    btLegendEle = [Line2D([0], [0], color="white", marker=shapes[i], lw=0, markeredgecolor=btc, label=btLabel) for i,(btLabel,btc) in enumerate(btColLab)]
...
def checkPairsAddSimple(adata, batchName, labelName):
    ...
    adata.obs["overlapLabel"] = pd.Categorical(findOverlapLabels(adata.uns["pairs"], adata.obs[labelName]))
    return(adata)
